...
class Renderer:
    ...
    def disconnect_mesh(self, vertices, faces):
    ...
    def render_normal(
        ...
        #face_normals = self.calc_face_normals(vertices, faces, normalize=True)
        #vert_normals = self.calc_vertex_normals(vertices, faces, face_normals)

        # ControlNet normal convention
        if model == 'ControlNet':
            vert_normals[..., 0] *= -1.0

        # interpolate vertex normals
        # - pixel_normals: num_cameras x h x w x 3
        pixel_normals,_ = dr.interpolate(vert_normals, rast_out, faces)
    ...
    def render_normal_with_normal_map(
        self,
        vertices,
        faces,
        normal_imgs,
        ...
        x = vertices_clip[idx,:,0]
        y = vertices_clip[idx,:,1]
        w = vertices_clip[idx,:,3]

        pixel_x = (x / w + 1.0) / 2.0 * self._image_size[0]
        pixel_y = (y / w + 1.0) / 2.0 * self._image_size[1]

        pixel_x = torch.round(pixel_x)
        pixel_y = torch.round(pixel_y)

        pixel_x = pixel_x.long()
        pixel_y = pixel_y.long()

        vert_normals = normal_imgs[idx,:,pixel_y, pixel_x]
        vert_normals = vert_normals.transpose(1,0).contiguous()
