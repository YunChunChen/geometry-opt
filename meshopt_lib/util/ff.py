...
def perspective_projection(r, device, l=None, t=None, b=None, n=1.0, f=50.0, flip_y=True):
    if l is None:
        l = -r
    if t is None:
        t = r
...
def orthographic_projection(r, device, l=None, t=None, b=None, n=1.0, f=50.0, flip_y=True):
    if l is None:
        l = -r
    if t is None:
        t = r
    ...
    p = torch.zeros([4,4],device=device)
    p[0,0] = 2/(r-l)
    p[1,1] = 2/(t-b) * (-1 if flip_y else 1)
    p[2,2] = -2/(f-n)
    p[3,0] = -(r+l)/(r-l)
    p[3,1] = -(t+b)/(t-b)
    p[3,2] = -(f+n)/(f-n)
...
def make_star_cameras(az_count,pol_count,distance:float=10.,r=None,image_size=[512,512],device='cuda'):
    ...
    return mv, perspective_projection(r,device)

def make_wonder3d_cameras(az_count,pol_count,distance:float=10.,r=None,device='cuda'):
    if r is None:
        r = 1/distance

    phi = torch.tensor([0.0, 45.0, 90.0, 180.0, -90.0, -45.0]) * -1.0 * torch.pi / 180.0
    mv = torch.eye(4, device=device).unsqueeze(0).expand(6,4,4).clone()
    mv[:,2,2] = phi.cos()
    mv[:,2,0] = -phi.sin()
    mv[:,0,2] = phi.sin()
    mv[:,0,0] = phi.cos()
...
'''
def make_wonder3d_cameras(az_count,pol_count,distance:float=10.,r=None,image_size=[512,512],device='cuda'):
    if r is None:
        r = 1/distance

    mv = torch.eye(4).unsqueeze(0).expand(6,4,4).clone()

    views = ['front', 'front_right', 'right', 'back', 'left', 'front_left']
    for i in range(6):
        txt_path = 'wonder3d_views/000_{}_RT.txt'.format(views[i])
        view_mat = np.loadtxt(txt_path, dtype=float)
        mv[i,:3,:4] = torch.tensor(view_mat)

    mv = mv.cuda()

    return mv, orthographic_projection(1/1.3,device)
'''
