...
class ObjaverseDataset(Dataset):
    ...
    def __getitem_joint__(self, index, debug_object=None):
        ...
        # num_views x 3 x 256 x 256
        img_tensors_in = [
            self.load_image(os.path.join(self.root_dir, object_name, 'rgb_%s.png' % (cond_view)), bg_color, alpha=None, return_type='pt').permute(2, 0, 1)
        ] * self.num_views
        ...
        for view, tgt_w2c in zip(view_types, tgt_w2cs):
            ...
            if self.read_color:                        
                img_path = os.path.join(self.root_dir, object_name, 'rgb_%s.png' % (view))
                img_tensor = self.load_image(img_path, bg_color, alpha=None, return_type="pt").permute(2, 0, 1)
                img_tensors_out.append(img_tensor)

            if self.read_normal:
                normal_path = os.path.join(self.root_dir, object_name, 'normals_%s.png' % (view))
                normal_tensor = self.load_normal(normal_path, bg_color, alpha=None, RT_w2c=tgt_w2c, RT_w2c_cond=cond_w2c, return_type="pt").permute(2, 0, 1)
