...
def log_vis(
    ...
    pred_img = torch.cat(pred_img, dim=0).to(input_img.device)
    ...
    error = torch.mean(torch.abs(pred_img - gt_img))
    accelerator.log({'{} reconstruction error'.format(name): error}, step=global_step)
...
def log_validation(
    ...
    for i, batch in enumerate(dataloader):
        ...
        rgb_in = torch.cat([rgb_in]*2, dim=0).to(dtype=weight_dtype) # 2B x Nv x 3 x H x W
        rgb_in = rearrange(rgb_in, "B Nv C H W -> (B Nv) C H W")

        normal_in = torch.cat([normal_in]*2, dim=0).to(dtype=weight_dtype) # 2B x Nv x 3 x H x W
        normal_in = rearrange(normal_in, "B Nv C H W -> (B Nv) C H W")
        ...
        log_vis(cfg, accelerator, rgb_in_all, rgb_pred_all, rgb_gt_all, '{} - RGB'.format(name), global_step)
        log_vis(cfg, accelerator, normal_in_all, normal_pred_all, normal_gt_all, '{} - Normal'.format(name), global_step)
...
def main(cfg):
    ...
    # new
    controlnet = MVControlNetModel.from_unet(unet)
    if accelerator.is_main_process:
        print("Initializing controlnet from pre-trained unet")

    if cfg.use_ema:
        ema_controlnet = EMAModel(controlnet.parameters(), model_cls=MVControlNetModel, model_config=controlnet.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse('0.16.0'):
        ...
        def load_model_hook(models, input_dir):
            if cfg.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, 'controlnet_ema'), MVControlNetModel)
            ...
            for i in range(len(models)):
                ...
                load_model = MVControlNetModel.from_pretrained(input_dir, subfolder='controlnet')
    ...
    #val_train_dataset = MVDataset(**cfg.train_dataset)
    ...
    if accelerator.is_main_process:
        print("ControlNet dtype: {}".format(accelerator.unwrap_model(controlnet).dtype))
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                normal_in = torch.cat([normal_in]*2, dim=0)
                ...
                normal_in  = rearrange(normal_in, "B Nv C H W -> (B Nv) C H W").to(weight_dtype)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % cfg.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        try:
                            controlnet.module.save_pretrained(os.path.join(cfg.output_dir, f'controlnet-{global_step}/controlnet'))
                        except:
                            controlnet.save_pretrained(os.path.join(cfg.output_dir, f'controlnet-{global_step}/controlnet'))
